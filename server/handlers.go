...
func (s *Server) getAudiobooks(w http.ResponseWriter, r *http.Request) {
	var err error
	queryTimeout := 30 * time.Second // Leave 10s buffer for WriteTimeout
...
func (s *Server) postAudiobook(w http.ResponseWriter, r *http.Request) {
	...
	if err = s.Repositories.AudiobookRepository.CreateAudiobook(ctx, &audio); err != nil {
		errs.HTTPErrorResponse(w, logger, err)
		return
	}
...
func (s *Server) editAudiobook(w http.ResponseWriter, r *http.Request) {
	...
	if err = s.Repositories.AudiobookRepository.EditAudiobook(ctx, &audioBD); err != nil {
		errs.HTTPErrorResponse(w, logger, errs.E(errs.ERR_ENCODEJSON, err))
		return
	}
...
func (s *Server) getLanguages(w http.ResponseWriter, r *http.Request) {
	...
	languages, err := s.Repositories.LanguageRepository.GetTTSLanguages(ctx, gender)
	if err != nil {
		errs.HTTPErrorResponse(w, logger, err)
		return
	}
