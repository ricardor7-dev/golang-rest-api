...
func ParseFlags(args []string, config *Config) error {
	...
	var (
		...
		dbhost        = fs.String("pghost", "localhost", fmt.Sprintf("postgresql database host (also via %s)", db.DBHostEnv))
		dbport        = fs.String("pgport", "5432", fmt.Sprintf("postgresql database port (also via %s)", db.DBPortEnv))
		dbname        = fs.String("pgdatabase", "", fmt.Sprintf("postgresql database name (also via %s)", db.DBNameEnv))
		dbuser        = fs.String("pguser", "", fmt.Sprintf("postgresql database user (also via %s)", db.DBUserEnv))
		dbpassword    = fs.String("pgpassword", "", fmt.Sprintf("postgresql database password (also via %s)", db.DBPasswordEnv))
		dbSSLMode    = fs.String("pgsslmode", "disable", fmt.Sprintf("postgresql database password (also via %s)", db.DBSSLMode))
	)
